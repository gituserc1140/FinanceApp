...
def generate_insights(user_id: int = 1) -> list[str]:
    ...
    if len(cashflow) >= 2:
        ...
        latest_rate = ((latest["income"] - latest["expenses"]) / latest["income"] * 100) if latest["income"] > 0 else 0
        previous_rate = ((previous["income"] - previous["expenses"]) / previous["income"] * 100) if previous["income"] > 0 else 0
